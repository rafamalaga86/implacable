...
// =========================================================
  ...
  $('.action-modify-modal').on('click', function(){
    var modal = $('#modal-modify-session');
    var row = $(this).parents('tr');
    var id = row.data('session-id');

    modal.find('form').attr('action', modal.find('form').data('action').replace('%session_id%', id));
    modal.find('.date').val(row.find('.date').data('date'));
    modal.find('.sets').val(row.find('.sets').text());
    modal.find('.reps').val(row.find('.reps').text());
    modal.find('.weight').val(row.find('.weight').text());
    modal.find('.level').val(row.find('.level').text());
  ...
  $('.action-open-modal-add-session').on('click', function(){
    ...
    var last_sets   = card.data('last-sets')   === '' ? null : card.data('last-sets');
    var last_reps   = card.data('last-reps')   === '' ? null : card.data('last-reps');
    var last_weight = card.data('last-weight') === '' ? null : card.data('last-weight');
    var last_level  = card.data('last-level')  === '' ? null : card.data('last-level');
    var exercise_id = card.data('exercise-id')  === '' ? null : card.data('exercise-id');

    modal.find('.copy-exercise').val(exercise_id);
    modal.find('.copy-sets').val(last_sets);
    modal.find('.copy-reps').val(last_reps);
